...
client.on("ready", () => {
  ...
  else cronUpdate.setTime(new CronTime(`0 0 * * * *`));
  cronUpdate.start();
  console.log(
...
client.on("messageCreate", (message) => {
  ...
  if (
    ...
    message.author.bot ||
    message.channel.id !== process.env.DISCORDJS_TEXTCHANNEL_ID
  )
    return;
  var args = [];
  ...
  switch (CMD_NAME.toUpperCase()) {
    ...
    case "ADD":
      {
        if (args.length === 0)
          return message.channel.send("Usage: `!add <URL> (<CSS SELECTOR>)`");
        var url = args[0];
        var selector = "head";
        if (args[1]) {
          selector = args[1];
        ...
        got(site.url)
          .then((response) => {
            ...
            //Get css element
            if (site.css)
              var content = dom.window.document.querySelector(
                site.css
              ).textContent;
            //Get head is no css element is selected
            else
              var content =
                dom.window.document.querySelector("head").textContent;
    ...
    case "REMOVE":
      {
        if (args.length === 0 || isNaN(args[0]))
          return message.channel.send("Usage: `!remove <NR [1-99]>`");
        if (args[0] < 1 || args[0] > 99 || args[0] > sitesToMonitor.length)
          return message.channel.send(
            "Not a valid number. Usage: `!remove <NR [1-99]>`"
          );
    ...
    case "UPDATE":
      {
        message.channel.send(
          `Updating \`${sitesToMonitor.length}\` site(s)...`
        );
        update();
      ...
      break;
    case "INTERVAL":
      if (args.length === 0 || isNaN(args[0]) || args[0] < 1 || args[0] > 60)
        return message.channel.send("Usage: `!interval <MINUTES [1-60]>`");
      {
        if (Math.round(args[0]) < 60) {
          cronUpdate.setTime(
            new CronTime(`0 */${Math.round(args[0])} * * * *`)
          );
        } else {
        ...
        message.channel.send(
          `Interval set to \`${cronTime.interval}\` minutes.`
        );
        cronUpdate.start();
      }
      break;
    ...
    case "STOP":
      {
        ...
        message.channel.send(
          "Paused website monitoring... Type `!start` to resume."
        );
      }
      break;
    case "STATUS":
      {
        ...
        if (cronUpdate.running)
          message.channel.send(
            `Site Watcher is running with an interval of \`${cronTime.interval}\` minute(s).`
          );
        else
          message.channel.send(
            "Site Watcher is not running. Use `!start` to start monitoring websites."
          );
      }
      break;
    default:
